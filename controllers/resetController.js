...
export const requestPasswordReset = async (req, res) => {
  try {
    ...
    if(existingRequest)
      return res.status(400).json({error: "Please wait 5 minutes before requesting another link."})
...
export const passwordChange = async (req,res) => {
  try{
      ...
      const resetRecord = await PasswordReset.findOne({ userId }).select('+tokenHash');

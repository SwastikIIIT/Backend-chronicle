...
export const getUserDetail = async (req, res) => {
  try {
    ...
    return res.status(200).json({
      user: user[0],
      message: "Successfully fetched user info from DB.",
    });
  } catch (err) {
    console.error("User fetch error", err);
    return res.status(500).json({
      error: err.message || "Failed to fetch user data from database",
    });
  }
};
...
export const uploadAvatar = async (req, res) => {
  try {
    ...
    return res
      .status(500)
      .json({ error: err?.message || "Failed to generate presigned URL" });
  }
};
...
export const saveAvatar = async (req, res) => {
  try {
    ...
    return res
      .status(500)
      .json({ error: err?.message || "Failed to save image." });
  }
};
...
export const setup2FA = async (req, res) => {
  try {
    const user = await User.findById(req.user.id).select(
      "+twoFactor.secret +twoFactor.enabled",
    );
    ...
    return res
      .status(500)
      .json({ error: err?.message || "Failed to initiate 2FA setup" });
  }
};
...
export const verify2FA = async (req, res) => {
  try {
    ...
    const user = await User.findById(userId).select(
      "+twoFactor.secret +twoFactor.enabled +twoFactor.recoveryCodes",
    );
    ...
    const dbRecoveryCodes = await Promise.all(
    ...
    return res
      .status(200)
      .json({ message: "Two-factor authentication enabled", recoveryCodes });
  } catch (err) {
    console.error("2FA verify error:", err);
    return res
      .status(500)
      .json({ error: err?.message || "Failed to enable 2FA" });
  }
};
...
export const disable2FA = async (req, res) => {
  try {
    ...
    return res
      .status(500)
      .json({ error: err?.message || "Failed to disable 2FA" });
  }
};
